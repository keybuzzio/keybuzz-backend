...
import { JobType, OutboundEmailStatus } from '@prisma/client';
...
export async function sendValidationEmail(connectionId: string, country?: string) {
  ...
  const addressesToValidate = country
    ...
    : connection.addresses.filter(a => a.marketplaceStatus !== 'VALIDATED');
  ...
  for (const address of addressesToValidate) {
    ...
    // Enqueue outbound email job with outboundEmailId
